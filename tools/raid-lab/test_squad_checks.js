...
const ex={duration:180,encounter_timeline:{model:'EX',special_interception:true,target_reached:true,reward_stage:9,simulated_until:42,survival:'survived modeled attacks',checks:[{status:'passed'}]}};
assert.equal(get(ex,'survival').value,'Stage 9 reached');
assert.equal(get(ex,'qte').state,'pass');
ex.encounter_timeline.checks=[];
assert.equal(get(ex,'qte').value,'Not encountered');
ex.encounter_timeline.qte_route_skipped=true;
assert.equal(get(ex,'qte').value,'Route avoided');
ex.encounter_timeline.stop_reason='First squad death';
assert.equal(get(ex,'survival').state,'fail');
const exMissile=structuredClone(ex);
exMissile.encounter_timeline.projectiles=[{kind:'hp',hp:100,max_hp:1000,status:'failed',deadline:5}];
assert.match(get(exMissile,'projectiles').detail,/1,000 HP/);
const exEarly=structuredClone(ex);
exEarly.encounter_timeline.target_reached=false;
exEarly.encounter_timeline.qte_route_skipped=false;
assert.equal(get(exEarly,'qte').value,'Not reached');
const exCore=structuredClone(ex);
delete exCore.encounter_timeline.stop_reason;
exCore.encounter_timeline.critical_deadlines_supported=true;
exCore.encounter_timeline.critical_parts=[{id:'Weapon_03',destroyed_at:10}];
assert.equal(get(exCore,'parts').value,'Core disabled');
