...
const RaidHistory = (() => {
 ...
 function create({root, api, feedback, getCatalog, download, checks, getModelRevision=()=>null}) {
  ...
  return {show() {if(!state.loaded || state.dirty) load();}, reset() {state.listToken++;state.detailToken++;Object.assign(state,{modes:[],mode:'',boss:'',runs:[],total:0,hasMore:false,loaded:false,dirty:true,detail:null});el('historyDetail').innerHTML='';el('historyDetail').hidden=true;el('historyBrowse').hidden=false;el('historyRuns').innerHTML='';el('historyStatus').innerHTML='';el('historyMore').hidden=true;populateFilters();}, invalidate() {state.dirty = true; if(!root.hidden && !state.detail) load();}, state};
 }
